...
controller = replace_once(
    ...
    '''\t\t\tcase 7:\n\t\t\tcase 8:\n\t\t\tcase 9:\n\t\t\tcase 10: return;\n''',
    'disable manual axis assignment'
)
...
controller = replace_once(
    ...
    'readable binding custom labels'
)
...
settings = replace_once(
    ...
    'refresh axes after clear config'
)
settings = replace_once(
    settings,
    ...
    'refresh axes after selecting config'
)
...
for logical, (widget_guid, slot_guid, widget_name) in axis_rows.items():
    old = f'''      ButtonWidgetClass "{{{widget_guid}}}" : "{{C9DF0E6590F6C388}}UI/layouts/WidgetLibrary/SpinBox/WLib_SpinBox.layout" {{\n       Name "{widget_name}"\n       Slot LayoutSlot "{{{slot_guid}}}" {{ Padding 4 4 4 4 }}\n       components {{ SCR_SpinBoxComponent "{{5472C6CBC0640458}}" {{ m_sLabel "{logical} Axis" m_sLabelLayout "{{F003823FF141983C}}UI/layouts/Menus/SettingsMenu/CustomWidgets/SettingsLabel.layout" m_bUseLightArrows 1 m_bShowHints 0 }} }}\n      }}\n'''
    ...
    layout = replace_once(layout, old, new, f'{logical} axis layout row')
